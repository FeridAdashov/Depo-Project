...
public class DepoSellsByMarketFragment extends Fragment implements DatePickerDialog.OnDateSetListener {

    private final ArrayList<String> nameList = new ArrayList<>();
    private final ArrayList<String> searchNameList = new ArrayList<>();
    private final HashMap<String, SellByMarketModel> sellList = new HashMap<>();
    ...
    private DepoSellsByMarketsRecycler adapter;
    ...
    private void configureFabButton() {
        ...
        imageView.setOnClickListener(v -> {
            ...
            transaction.replace(R.id.container, new DepoSellsBySellerFragment());
            transaction.commit();
        });
    }
    ...
    private void configureListViewDepo() {
        adapter = new DepoSellsByMarketsRecycler(activity, nameList, searchNameList, sellList);
                ...
                for (DataSnapshot snapshotSellDates : dataSnapshot.getChildren()) {
                    ...
                    for (DataSnapshot snapshotUsers : snapshotSellDates.getChildren()) {
                        ...
                        for (DataSnapshot snapshotTimes : snapshotUsers.getChildren()) {
                            ...
                            SellByMarketModel model = sellList.get(key);
                            if (model != null) {
                                model.sum += sum;
                                model.times.add(snapshotTimes.getKey());
                            } else {
                                int index = 0;
                                ...
                                sellList.put(key, new SellByMarketModel(user, sum, date, snapshotTimes.getKey()));
                            }
                        }
                    }
                }
