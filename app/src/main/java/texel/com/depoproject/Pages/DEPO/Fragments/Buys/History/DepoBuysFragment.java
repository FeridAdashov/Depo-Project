...
public class DepoBuysFragment extends Fragment implements DatePickerDialog.OnDateSetListener {
    ...
    private DepoRecyclerBuyHistory adapter;
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        drBuys = DatabaseFunctions.getDatabases(activity).get(0).child("BUYS");
    ...
    private void configureListViewDepo() {
        adapter = new DepoRecyclerBuyHistory(activity, dateList, searchDateList, buyValueList, percentValueList);
                ...
                for (DataSnapshot snapshotDates : dataSnapshot.getChildren()) {
                    ...
                    if (SharedClass.checkDate(begin, end, child_date)) {
                        ...
                        if (sum != null) {
                            ...
                            paidSum = SharedClass.twoDigitDecimal(paidSum + sum);
                            percentSum = SharedClass.twoDigitDecimal(percentSum + percentValue);
                            commonSum = SharedClass.twoDigitDecimal(commonSum + sum + percentValue);
                        }
                    }
                }
                if (dateList.size() > 0) {
