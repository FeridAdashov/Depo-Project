...
public class DepoSellsByMarketsRecycler extends RecyclerView.Adapter<DepoSellsByMarketsRecycler.MyViewHolder> {

    private final ArrayList<String> nameList;
    private final ArrayList<String> searchNameList;
    private final HashMap<String, SellByMarketModel> sellList;
    private final Activity activity;

    public DepoSellsByMarketsRecycler(Activity activity,
                                      ArrayList<String> nameList,
                                      ArrayList<String> searchNameList,
                                      HashMap<String, SellByMarketModel> sellList) {
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View listItem = LayoutInflater.from(parent.getContext()).inflate(R.layout.card_view_sells_by_market_depo, parent, false);
    ...
    public void onBindViewHolder(MyViewHolder holder, final int pos) {
        ...
        SellByMarketModel sell = sellList.get(key);

        holder.name.setText(key);
        holder.sum.setText(String.format("%.2f", sell.sum));

        holder.sells.setOnClickListener(v -> showTimesDialog("SELLS/" + sell.date + "/" + sell.seller, sell.times));
    }
